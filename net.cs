...
public class net  {
	...
	public void FeedForward(float[] inputValues)
	{
		...
		//forward propagation
		for (int layerNum = 1; layerNum < m_layers.Count; layerNum++) {//from the first hidden layer on

			Layer previousLayer = m_layers [layerNum - 1];//set the new output for each neuron in this layer
			for (int n = 0; n < m_layers[layerNum].neuronas.Count-1; n++) {//except for bias, cause bias output value is always 1
	...
	public void BackProp(float[] targetValues){
		...
		for (int layerNum = m_layers.Count-2; layerNum > 0; layerNum--) {//DEBUG CHECK IF layerNum >0 , o layerNum>=0
	...
	public string ExportToCSV(){
		string topology="";
		for (int layerNum = 0; layerNum< m_layers.Count; layerNum++) {// for each layer, add the number of neurons in topollogy string
		...
		topology += "\n";
		string neurons = "";
		for (int layerNum = 0; layerNum< m_layers.Count; layerNum++) {// for each layer save the neurons weights in string
			...
			for (int neuron = 0; neuron < m_layers[layerNum].neuronas.Count; neuron++) {
				...
				for (int weightNum= 0;weightNum < currentNeuron.neuronWeights.Count; weightNum++) {// all the neuron weights in this neuron
					weightsString+= currentNeuron.neuronWeights[weightNum].weight;
					if (weightNum < currentNeuron.neuronWeights.Count - 1) {//add separator, unless is the last weight
						weightsString += ",";
					}
				}
	...
	//	Debug.LogWarning (WeightsCVSstring);
		...
		for (int layer = 0; layer < m_layers.Count-1; layer++) {//every layer except for the output

			for (int n = 0; n < m_layers[layer].neuronas.Count; n++) {// for each neuron in the layer
				//current neuron
...
public class Neuron  {
	...
	float m_neuronOutput,m_gradient,eta=0.2f,alpha=0.5f;
	int neuronIndex;
	...
	public void FeedForward(Layer previousLayer){
		...
		for (int n = 0; n < previousLayer.neuronas.Count; n++) {
			sum += previousLayer.neuronas [n].GetOutputValue () *
	...
	public void LoadCSVWeights(string CSVWeights){
		...
		for (int w = 0; w < weightString.Length; w++) {//for every weight assign the weight to the network
			float readWeight=0;
			if (!float.TryParse (weightString[w], out readWeight)){// try to convert the text to an float number
...
public class Layer  {public List<Neuron> neuronas= new List<Neuron>{};}
public class Connections {public float weight,deltaWeight;}